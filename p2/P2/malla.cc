...
void ObjMallaIndexada::draw(int modo)
{
  ...
  if (modo == 0){
    draw_ModoInmediato();
    cout << "cambio a inmediato" << endl;
  }else if (modo == 1){
    draw_ModoDiferido();
    cout << "cambio a diferido" << endl;
  }else if (modo == 2){
    modoAjedrez();
    cout << "cambio a ajedrez" << endl;
  }
...
void ObjRevolucion::tapaSur(int tam_perfil_vertices, int num_vueltas){
  ...
  for (int i = 0; i < num_vueltas; ++i){
    ...
    if ((i+1) == num_vueltas){
      triangulo_tapa_sur = {vertices.size()-unidad_2, i*tam_perfil_vertices, 0};
    }else{
      triangulo_tapa_sur = {vertices.size()-unidad_2, i*tam_perfil_vertices, (i+1)*tam_perfil_vertices};
    }
...
Esfera::Esfera(const int num_vertices_perfil, const int num_instancias_perfil){
  ...
  for (int i = 0; i < num_vertices_perfil; ++i){
    ...
    float y = sin(alfa);
    float x = cos(alfa); //tambien x = sqrt(1-y*y);
